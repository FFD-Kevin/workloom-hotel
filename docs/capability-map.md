# WorkLoom Hotel 能力地图（Capability Map）
...
| 能力 | 调用 | 验证 |
|---|---|---|
| 孪生模拟 | `pnpm demo:twin` | 见 docs/DEMO-TWIN.md |
| 孪生快照 / 恢复 | `pnpm demo:twin:snapshot` / `pnpm demo:twin:restore` | 快照文件生成/回滚 |
| 孪生实时精灵 | `pnpm demo:twin:live` | 实时事件流输出 |

## L2 验证层 · 质量与发布纪律（硬性）

| 能力 | 调用 | 验证 |
|---|---|---|
| 主测试套件 | `pnpm suite` | 全绿，exit 0（H-05/07/09 为既有 onboarding 遗留，见 AUDIT） |
...
| 孪生演示文档 | `docs/DEMO-TWIN.md` / `docs/TWIN-COVERAGE.md` | 数字孪生口径 |
...
5. **demo:twin 系列**——本仓特有的数字孪生演示，向客户展示时直接用。
6. **skills/official/** + **docs/demo 演示页**——自带资产，别临时重造。
