...
| D13 | **dsh 社区生态三借鉴纳入计划**（2026-08-17 评估 Datawhale 教程：社区项目 DSH Desktop ≈8.5k★ / dsh-TUI ≈1.5k★ 均不改 Harness 内核、纯消费官方文档化 seam 构建前端，外部实证 D12 双轨路线可行）：① 官方 headless profile（`dsh --profile headless "任务"` 非交互一次性执行、打印结果即退出）纳入阶段四回归门禁（新增 E6 卡），并作为 H-5（kill -9 重放续跑且幂等）的验收载体；② dsh-TUI `/doctor` 一屏自检清单（Node 版本/架构/模型/工作目录/凭据状态/会话存储）并入 E2 doctor.sh 设计；③ DSH Desktop 打包模式（Electron 内嵌 Node + Harness 服务生命周期托管 + 首启自动建 Profile，参考 anywhere-labs/deepseek-harness-desktop）进停车场，触发条件=私有化交付/一键分发需求出现 | 架构与排期不变，仅阶段四增量；vendor fork 锁 rc.6 + 只依赖文档化 seam + 不追上游 `@latest` 的纪律不变；dsh-TUI 的斜杠命令交互体系不借鉴（自研舰桥前端交互语言不同） |
| D14 | **IM 通道接入采用 dsh-im 插件（D7 停车场触发条件达成：用户 2026-08-17 明确提出真实企业通道接入需求）**。核验事实（2026-08-17）：`xmanrui/dsh-im` 真实存在，npm `@xmanrui/dsh-im@0.2.2`（MIT，THIRD_PARTY_NOTICES 随包；当日 0.1.0→0.2.2 五连发=早期快速演进）；宿主插件 inject `connection/credentials/webServer` 全为文档化 seam、不改 Harness 内核（与 D12/D13 双轨纪律同构）；凭据单向提交本机 Host、RPC 不回传 Secret、管理 RPC 默认仅回环——与铁律 7 / L7.3 凭据引用口径相容。接入边界：dsh-im 只作 **L1 通道适配层**（消息进出 dsh 会话，入站留痕由 D12 既定 sessionPersistence 事件桥投影覆盖）；**L2 自研 `packages/base/im-channels`** 承载通道注册表（对账 approvals.channel 枚举）、入站归一化→五元事件（经网关三段瀑布）、审批卡片出站（D7 本地回环升级为多通道）、手势回调→`decide()` 复用（L5.2/L5.3 全继承）、openid→成员映射（members.im_openids，E5.2 预留位）。**首批仅启用官方 SDK 三通道 dingtalk/wecom/feishu**（与 approvals.channel 枚举对齐）；微信（iLink 长轮询）与 WhatsApp（baileys 非官方协议）有合规/封号风险→观察名单不启用；QQ/Telegram/Discord 次批评估。锁版本纪律同 D12：pin `0.2.2` + integrity 入库（`vendor/dsh-im/VENDOR.md`），不追 `@latest`；真实通道凭据由用户在 dsh 设置页自配（扫码/手动），WorkLoom 默认 `IM_DRIVER=mock` 全流程可跑（D4 同纪律） | Slack 不在 dsh-im 渠道内（保留枚举位，后续按同一通道适配器结构补）；dsh-im 快速演进风险由锁版本+契约测试（升级前全绿）兜住；阶段三前端游标（F10）不受影响，B11 为阶段二后端增量卡 |
