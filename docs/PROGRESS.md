...
| 阶段三 前端页面 | 🚧 F1–F6 完成（P1/P2/P9/P3 四页真实接线）；下一卡 F7=P4 | —（F12 后打 `v0.3.0`） |
| 阶段四 联调与启动脚本 | ⬜ 未开始 | — |
...
- **下一步**：**阶段三 F7 = P4 决断队列**（统一审查面板：队列分级（必审/逐步审/双人）+ diff 对照表 + 三手势 + 「为什么这样改」决策链路 + IM 卡片同步位；状态变体 p4/p4_empty/p4_conflict 快照冲突 E5.3）。后续：P5→P8→P6→P7 → F12 → tag `v0.3.0`。
