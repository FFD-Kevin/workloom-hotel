...
- **下一步**：**B9 night-shift**：候选清单（18:00）+ 状态机（unconfigured→ready→running⇄paused→package_generated）+ 一键暂停 ≤60s 取消链（G5）+ 08:30 决策包三段投影（F4.4/H-7 纯日志投影）+ 触发器引擎（F4.7）。首个文件：`packages/base/night-shift/scheduler.ts`。
