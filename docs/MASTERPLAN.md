...
| B11 | **IM 通道接入（D14，停车场触发）**：dsh-im 锁版集成（`@xmanrui/dsh-im@0.2.2` 官方三通道 dingtalk/wecom/feishu）+ 自研 `packages/base/im-channels` 通道域（注册表对账 approvals.channel / 入站归一化→五元事件经网关瀑布 / 审批卡片出站 / 手势回调复用 decide / im_openids 映射 E5.2）+ Mock 通道驱动（无凭据全流程可跑，D4 同纪律） | 入站消息 100% 落五元事件可检索（G8）；重复通道消息幂等丢弃（L1.4 同口径）；手势回调重复只处理首次（L5.3）；未映射 openid 无权审批被拒；`scripts/install-im-channels.sh` pin 安装 + integrity 校验 |
...
| ~~IM 连接器（钉钉/企微/飞书/Slack）~~ **已触发落地（D14，2026-08-17）**：钉钉/企微/飞书经 dsh-im 接入（B11）；Slack 保留枚举位待补 | 触发条件已达成（用户提出真实企业通道接入需求）；微信/WhatsApp 非官方协议通道仍在观察名单不启用 |
