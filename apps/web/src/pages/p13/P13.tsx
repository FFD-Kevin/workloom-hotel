...
export default function P13() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const r = (await trpc.twin.events.query({ actions: ["order.confirm", "order.reconcile", "order.refund"], limit: 80 })) as unknown as Ev[];
      if (stop) return;
      ...
      {!ready ? (<><SkeletonBlock lines={2} h={44} /><SkeletonBlock lines={4} /></>) : (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            ...
            <Stat label="退款（R4 必审）" value={refunds.length} tone="text-warn" hint="≥¥500 挂起审批" />
          </div>
          ...
          {orders.slice(0, 25).map((ev) => (
            <Row key={ev.event_id} time={fmtTime(ev.context.time)}
              ...
              <span className="text-ink3"> · {ev.context.channel ?? "直连"} · {ev.decision.action === "order.confirm" ? `已确认 ✓（可售校验通过）` : ev.decision.action === "order.reconcile" ? "对账一致 ✓" : "退款审批"}</span>
