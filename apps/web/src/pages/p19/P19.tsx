...
const QUESTIONS: Array<{ key: Q; label: string }> = [
  ...
  { key: "occ_trend", label: "近 30 天 OCC/ADR/RevPAR 走势" },
    ...
    <Bridge left={<PageNav current="P19" />}>
      ...
      {!ready || !result ? (<><SkeletonBlock lines={2} h={44} /><SkeletonBlock lines={4} /></>) : (
        <div className="space-y-3">
          {q === "channel_revenue" && (
            <>
              <SystemDivider time="渠道收入" summary="order.confirm 事件实时聚合（订单量 × 金额）" />
              {result.rows.map((r) => (
              ...
              <Note>💡 直连渠道零佣金：净贡献率最高——提升直连占比是 retention-manager 的持续建议方向。业主月报可一键导出（finance-reporting）。</Note>
            </>
          )}
          {q === "occ_trend" && (
            <>
              ...
              {[...result.rows].reverse().map((r) => (
                <HBar key={String(r.date)} label={String(r.date).slice(5)} pct={Math.round(Number(r.occ ?? 0) * 100)}
                  value={`OCC ${(Number(r.occ ?? 0) * 100).toFixed(0)}% · ADR ¥${r.adr ?? "—"}`} tone="bg-holo" />
              ))}
            </>
          )}
          {q === "price_attribution" && (
