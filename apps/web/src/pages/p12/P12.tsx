  ...
  const right = (
    <>
      ...
      <div className="rounded-lg border border-line bg-card p-3 text-xs leading-relaxed text-ink2">
        ...
        <div>OCC <b className="text-gold">{pct(goals?.year?.occ)}</b> · ADR <b className="text-gold">¥{goals?.year?.adr ?? "—"}</b> · RevPAR <b className="text-gold">¥{goals?.year?.revpar ?? "—"}</b></div>
        <div>差评率 ≤ <b>{pct(goals?.year?.bad_review_rate)}</b> · 复购率 <b>{pct(goals?.year?.repurchase_rate)}</b></div>
      </div>
      <div className="mb-2 mt-3 px-1 text-[11px] tracking-[.2em] text-ink3">渠道分解 · CHANNELS</div>
      ...
      <div className="mt-3 rounded-lg border border-gline bg-card p-3 text-xs leading-relaxed text-ink3">
        目标从年初口号变成每周作战：达成偏离时序进度自动归因，并联动 revenue-manager 生成补救建议（review 级）。
      </div>
    </>
  );
  ...
  return (
    <Bridge left={<PageNav current="P12" />} right={right}>
      ...
      {!ready ? (
        ...
        <EmptyState icon="🎯" title="尚未设定经营目标" hint="使用目标设定向导：年度目标 → 自动分解到月/渠道/房型（可一键采用同档门店基准值）。" />
      ) : (
        <div className="space-y-3">
          {/* 月度目标卡 */}
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg border border-line bg-card p-3">
              <div className="text-[11px] text-ink3">月营收目标</div>
              <div className="mt-1 text-h1 font-black text-gold">{wan(monthGoal?.revenue)}</div>
              <div className="mt-1 text-[11px] text-ink3">当前 <b className="text-ink2">{wan(revT?.actual)}</b></div>
            </div>
            <div className="rounded-lg border border-line bg-card p-3">
              <div className="text-[11px] text-ink3">OCC 目标 vs 实际</div>
              <div className="mt-1 text-h1 font-black text-ink2">{pct(monthGoal?.occ)} <span className="text-[13px] text-ink3">/ {pct(occT?.actual)}</span></div>
              <div className="mt-1 text-[11px]">
                {occT?.pace === "on_track" ? <span className="text-go">● 进度正常</span> : <span className="text-warn">● 落后时序</span>}
              </div>
            </div>
            <div className="rounded-lg border border-line bg-card p-3">
              <div className="text-[11px] text-ink3">ADR 目标</div>
              <div className="mt-1 text-h1 font-black text-ink2">¥{monthGoal?.adr ?? "—"}</div>
              <div className="mt-1 text-[11px] text-ink3">RevPAR 年目标 ¥{goals?.year?.revpar ?? "—"}</div>
            </div>
          </div>
          {/* 时序进度条 */}
          <div className="rounded-lg border border-line bg-card p-3">
            <div className="flex justify-between text-[11px] text-ink3"><span>OCC 达成进度（目标达成率）</span><span className="font-mono text-gold">{pacePct}%</span></div>
            ...
            return (
              <div key={t.event_id} className="rounded-lg border border-line bg-card p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-ink2">OCC {pct(a.occ?.actual)}（目标 {pct(a.occ?.target)}）</span>
                  <span className={`text-[11px] ${behind ? "text-warn" : "text-go"}`}>{behind ? "⚠ 落后时序" : "✓ 进度正常"}</span>
                  <span className="flex-1" />
                  <span className="font-mono text-[11px] text-ink3">营收 {wan(a.revenue?.actual)}</span>
                </div>
