...
export function StarRing() {
  ...
  return (
    <>
      ...
      {/* Ask 引导标签（常显，让全局入口「非常明显」） */}
      <div className="pointer-events-none fixed bottom-9 right-24 z-50 animate-pulse-hud rounded-full border border-gline bg-card/90 px-3 py-1.5 text-caption font-black tracking-wider text-gold shadow-[0_0_18px_rgba(255,160,60,.35)]">
        ✦ 有事点这里 Ask
      </div>
      {/* 悬浮球（64px；金色呼吸光圈；有待批时呼吸加速 + 红点 badge） */}
      <button
        type="button"
        onClick={onBallClick}
        onDoubleClick={onBallDoubleClick}
        title="星环 · 全局 Ask（⌘K 唤起 · 双击进剧场）"
        className="fixed bottom-6 right-6 z-50 flex h-16 w-16 cursor-pointer items-center justify-center rounded-full gold-grad shadow-[0_0_36px_rgba(255,160,60,.65)]"
      >
        <span
          aria-hidden
